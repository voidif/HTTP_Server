...
var index = {
    ...
    init: function() {
        ...
        this.time.innerHTML = new Date().toLocaleTimeString();
        this.getRate();
    ...
    getRate: function() {
        ...
        xmlhttp.onreadystatechange = function(){
            if (xmlhttp.readyState == 4){
                ...
                var text = xmlhttp.responseText;
                var obj = JSON.parse(text);
                index.rate.innerHTML = obj.USD_CNY;

                //update time
                var myDate = new Date();
                index.time.innerHTML = myDate.toLocaleTimeString();
            }
        }
        xmlhttp.open("GET","/json?id=rate",true);
        xmlhttp.send();
    ...
    readBlog: function(html) {
        ...
        xmlhttp.onreadystatechange = function(){
            if (xmlhttp.readyState == 4){
                ...
                var text = xmlhttp.responseText;
                var converter = new showdown.Converter()
                this.maindispaly.innerHTML = converter.makeHtml(text);
                this.flag = -1;
            }
        }
        var url = html.getAttribute("blogurl");
